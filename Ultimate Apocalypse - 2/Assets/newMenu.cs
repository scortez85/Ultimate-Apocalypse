...
public class newMenu : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("Submit") != 0)
        {
            ...
            else if (menuChoice == 1 && userName != "")
            {
                ...
                {
                    //if (playerList[k] != userName)//check to see if user/player name already exist
                    {


                        System.IO.Directory.CreateDirectory("Assets\\gameData\\players\\" + userName);
                        //System.IO.Directory.CreateDirectory("Assets\\gameData\\players\\" + userName + "\\" + userName);
                        playerPath = "Assets\\gameData\\players\\" + userName + "\\";

                        //create files
                        saveFile();
                        //GameObject.FindGameObjectWithTag("GameController").GetComponent<gameControl>().playerName = userName;
                        Application.LoadLevel("SubwayAssets");
                    }
                }
            }
    ...
    public void saveFile()
    {
        ...
        //companions Zachelion, Ellisa, Ava, Kali, Enacra, Maliki
...

        {
            ...
            //write chem to file
            for (int i = 0; i < userChem.Count; i++)
            {
                userChemWriter.WriteLine(userChem[i]);
    ...
    public void newUser()
    {
        ...
        //userScrap.Add("50");
        //player chem
        userChem.Add("-----list for chemicals-----");
    }
}
